...
def create_test_list(listSpec, n, radix=2):
    ...
    4: an alternative list that is difficult for merge sort, for example [0,2,1,3], [0,4,2,6,1,5,3,7]
    '''
    l = []
    listType = listSpec[0]
    if radix > 1:
    ...
    if listType == 0:
        ...
        if len(listSpec) > 1:
            ...
            stdVariance = 0.0125
        l = [int(N/2 + N*random.gauss(mu, stdVariance)/2) for i in xrange(N)]
        minValue = min(l)
        ...
        if len(listSpec) > 1:
            ...
            stdVariance = N/8
        l = [int(N/2 + random.gauss(mu, stdVariance)) for i in xrange(N)]
        minValue = min(l)
        l = [elem - minValue for elem in l]
    return l       
...
class ConcatenationExpression:
    ...
    the operators can be one of two choice: SimpleConcatenation acnd
    BucketConcatenation
    """
