...
class DataSet(Set):
    ...
    def set_values(self, values=None, *args, **kwargs):
        """
        Use the arguments `values`, `args` and `kwargs` to build
        up a mapping from the name to value. After that, the `set_value`
        method of each element is called provoked to set value for them.
        ...
        if (values is not None) :
            # check type of values to get correctly the values
